...
export const dashboardApi = {
  ...
  getSpendingTrends: async (period: 'week' | 'month' | 'year' = 'month'): Promise<SpendingTrendsResponse> => {
    const response = await apiClient.get<SpendingTrendsResponse>('/api/dashboard/spending-trends', {
      params: { period },
    });
    return response.data;
  },
  ...
  getTopMerchants: async (period: 'week' | 'month' | 'year' = 'month'): Promise<TopMerchantsResponse> => {
    const response = await apiClient.get<TopMerchantsResponse>('/api/dashboard/top-merchants', {
      params: { period },
    });
    return response.data;
  },
